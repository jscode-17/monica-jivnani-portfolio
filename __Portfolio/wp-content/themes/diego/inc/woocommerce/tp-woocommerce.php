...
add_action('woocommerce_single_product_summary','diego_product_single_features',60);

add_action('woocommerce_archive_description','woocommerce_breadcrumb',20);

add_filter('diego_woo_compare_filter', 'diego_woo_compare_func');
...
if(class_exists('WeDevs_Dokan')){
    function vendor_more_products_tab( $tabs ) {
        if ( check_more_seller_product_tab() ) {
            $tabs['more_seller_product'] = [
                'title'    => __( 'More Products From Seller', 'diego' ),
                'priority' => 99,
    ...
    function tp_get_more_products_from_vendor( $seller_id = 0, $posts_per_page = 6 ) {
        ...
        if ( $products->have_posts() ) {
            ...
            esc_html_e( 'No product has been found!', 'diego' );
        }
     ?>
        </div>
    <?php
...
// coupon
function diego_woo_compare_func(){
...
function diego_product_single_features(){

    $features_list = function_exists('tpmeta_field') ? tpmeta_field('diego_product_single_fea_meta') : '';

    $payment_switch = get_theme_mod('diego_product_single_payment_switch', false);
    $payment_text = get_theme_mod('diego_product_single_payment_text', 'Guaranteed safe & secure checkout');
    $payment_img = get_theme_mod('diego_product_single_payment_img');


    ?>

    <?php if(!empty($features_list[0]['diego_product_signle_fea'])) : ?>
    <div class="tp-product-details-msg mb-15">
        <ul>
            <?php foreach ($features_list as $feature) :?>
                <li><?php echo diego_kses($feature['diego_product_signle_fea']); ?></li>
            <?php endforeach; ?>
        </ul>
    </div>
    <?php endif; ?>
    ...
    <div class="tp-product-details-payment d-flex align-items-center flex-wrap justify-content-between">
        ...
        <img src="<?php echo esc_url($payment_img); ?>" alt="<?php echo esc_attr__('payment-img', 'diego'); ?>">
        <?php endif; ?>
    </div>
    <?php endif; ?>
...
*************************************************/
function diego_shipping_progress_bar() {
...
if(get_theme_mod( 'shipping_progress_bar_location_card_page',false) == true){
    add_action( 'woocommerce_before_cart_table',  'diego_shipping_progress_bar' );
}

if(get_theme_mod( 'shipping_progress_bar_location_mini_cart',false) == true){
    add_action( 'woocommerce_before_mini_cart_contents', 'diego_shipping_progress_bar' );
}

if(get_theme_mod( 'shipping_progress_bar_location_checkout',false) == true){
    add_action( 'woocommerce_checkout_before_customer_details', 'diego_shipping_progress_bar' );
}
...
function diego_sale_percentage(){
   global $product;
   $output = '';
   $icon = esc_html__("-",'diego');
...
if ( ! function_exists( 'diego_header_add_to_cart_fragment' ) ) {
    function diego_header_add_to_cart_fragment( $fragments ) {
        ob_start();
        ?>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'diego_header_add_to_cart_fragment' );

// diego_shopping_cart
function diego_shopping_cart(){
    if(is_null(WC()->cart)){
        return;
    }
    ob_start();
             ...
             <div class="cartmini__top-wrapper">
                 <div class="cartmini__top p-relative">
                     <div class="cartmini__top-title">
                         <h4><?php print esc_html__('Shopping cart','diego'); ?></h4>
                     </div>
               ...
               <div class="col">
                  <div class="tp-mobile-item text-center">
                     <a href="<?php echo esc_url(get_permalink(get_option( 'woocommerce_shop_page_id' ))); ?>" class="tp-mobile-item-btn">
                        <i class="diegoicon-store"></i>
                        <span><?php echo esc_html__('Store', 'diego'); ?></span>
                     </a>
                  </div>
               </div>
               <?php endif; ?>
               <div class="col">
                  <div class="tp-mobile-item text-center">
                     <button class="tp-mobile-item-btn tp-search-open-btn">
                        <i class="diegoicon-magnifying-glass"></i>
                        <span><?php echo esc_html__('Search', 'diego'); ?></span>
                     </button>
                  </div>
               </div>
               ...
               <div class="col">
                  <div class="tp-mobile-item text-center">
                     <a href="<?php echo esc_url( $wishlist_data::get_url( $key, true ) ); ?>" class="tp-mobile-item-btn">
                        <i class="diegoicon-heart"></i>
                        <span><?php echo esc_html__('Wishlist', 'diego'); ?></span>
                     </a>
                  </div>
               </div>
               <?php endif; ?>
               ...
               <div class="col">
                  <div class="tp-mobile-item text-center">
                     <a href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" class="tp-mobile-item-btn">
                        <i class="diegoicon-user"></i>
                        <span><?php echo esc_html__('Account', 'diego'); ?></span>
                     </a>
                  </div>
               </div>
               <?php endif; ?>
               <div class="col">
                  <div class="tp-mobile-item text-center">
                     <button class="tp-mobile-item-btn tp-offcanvas-open-btn">
                        <i class="diegoicon-menu-1"></i>
                        <span><?php echo esc_html__('Menu', 'diego'); ?></span>
                     </button>
                  </div>
               </div>
            </div>
...
add_filter('woocommerce_single_product_image_thumbnail_html', 'diego_product_img', 10, 2 );
function diego_product_img($html, $id){
    
    $video_url = function_exists('tpmeta_field')? tpmeta_field('diego_product_signle__video') : '';
...
if( !function_exists('diego_content_single_details') ) {
    function diego_content_single_details( ) {
        global $product;
        global $post;
        global $woocommerce;
        ...
        $enable_category = get_theme_mod('diego_product_single_category_switch', true);
        $enable_stock = get_theme_mod('diego_product_single_in_stock_switch', true);
        $enable_flash_sale = get_theme_mod('diego_product_single_flash_sale_switch', false);
        $enable_flash_sale_text = get_theme_mod('diego_product_single_flash_sale_text', 'Flash Sale Ends in:');
        $enable_stock_left = get_theme_mod('diego_product_single_stock_left_switch', '49');
        $enable_stock_left_count = get_theme_mod('diego_product_single_stock_left_count', '49');
    ...
    ?>
            ...
            <div class="tp-product-details-inventory d-flex align-items-center mb-10">
                ...
                <div class="tp-product-details-rating-wrapper d-flex align-items-center mb-10">
                    ...
                    <div class="woocommerce-product-rating tp-product-details-reviews">
                        ...
                        <?php //phpcs:disable ?>
                        <a href="#reviews" class="woocommerce-review-link" rel="nofollow"><?php printf( _n( '( %s Review )', '( %s Reviews )', $review_count, 'diego' ), '<span class="count">' . esc_html( $review_count ) . '</span>' ); ?></a>
                        <?php // phpcs:enable ?>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <?php if(!empty($enable_flash_sale)) : ?>
                ...
                <div class="tp-product-details-countdown d-flex align-items-center justify-content-between flex-wrap mt-25 mb-25">
                    ...
                    <div class="tp-product-details-countdown-time" data-countdown data-date="<?php echo esc_attr($_sale_date_end); ?>">
                        <ul>
                            <li><span data-days><?php echo esc_html__('0', 'diego'); ?></span><?php echo esc_html__('D', 'diego'); ?></li>
                            <li><span data-hours><?php echo esc_html__('0', 'diego'); ?></span><?php echo esc_html__('H', 'diego'); ?></li>
                            <li><span data-minutes><?php echo esc_html__('0', 'diego'); ?></span><?php echo esc_html__('M', 'diego'); ?></li>
                            <li><span data-seconds><?php echo esc_html__('0', 'diego'); ?></span><?php echo esc_html__('S', 'diego'); ?></li>
                        </ul>
                    </div>
                </div>
            ...
            <?php if(!empty($enable_stock_left)) : ?>
                ...
                <div class="tp-product-details-stock-bar mb-25">
                    <p><?php echo esc_html__('Hurry! Only', 'diego'); ?> <span><?php echo esc_html($stock_quantity); ?></span> <?php echo esc_html__('units left in stock!', 'diego'); ?> </p>
...
add_action( 'woocommerce_single_product_summary', 'diego_content_single_details', 4 );



// product-content archive
if( !function_exists('diego_content_product_grid') ) {
    function diego_content_product_grid( ) {


        if(get_theme_mod('shop_grid_layout') === "2"){
            diego_product_grid_type_2();
        }elseif (get_theme_mod('shop_grid_layout') === "3") {
            diego_product_grid_type_3();
        }elseif (get_theme_mod('shop_grid_layout') === "4") {
            diego_product_grid_type_4();
        }elseif (get_theme_mod('shop_grid_layout') === "5") {
            diego_product_grid_type_5();
        }elseif (get_theme_mod('shop_grid_layout') === "6") {
            diego_product_grid_type_6();
        }
        else{
            diego_product_grid_type_1();
        }
    }
}
add_action( 'woocommerce_before_shop_loop_item', 'diego_content_product_grid', 10 );


add_action('diego_product_badge_action', 'diego_product_badges');

function diego_product_badges(){
    global $product;
    global $post;
    global $woocommerce;
    $rating = wc_get_rating_html($product->get_average_rating());
    ...
    $is_product_on_trending = function_exists('tpmeta_field')? tpmeta_field('diego_product_on_trending') : '';
    $is_product_on_hot = function_exists('tpmeta_field')? tpmeta_field('diego_product_on_hot') : '';

    $product_bage_from_meta = function_exists('tpmeta_field')? tpmeta_field('diego_product_bage_type') : '';
        ...
        <ul class="d-flex">
            ...
            <?php if($enable_trending_badge || $is_product_on_trending === "on") : ?>
                ...
                <?php if($is_product_on_trending === "on") : ?>
                    <li>
                        <span class="onsale on-trending"><?php echo esc_html__('Trending', 'diego'); ?></span>
                    </li>
            ...
            <?php if($is_product_on_hot === "on") :?>
                <li>
                    <span class="onsale on-hot"><?php echo esc_html__('Hot', 'diego'); ?></span>
                </li>
...
add_action('diego_product_countdown_action', 'diego_product_countdown');

function diego_product_countdown(){
    global $product;
    global $post;
    global $woocommerce;

    $enable_flash_sale = get_theme_mod('diego_product_sale_countdown_switch', true);
...
function diego_product_grid_type_1(){
        ...
        <div class="tp-product-item-2 mb-40">
            ...
            <div class="tp-product-thumb-2 p-relative z-index-1 fix w-img">
                ...
                <div class="tp-product-badge-2 d-flex">
                    <?php do_action('diego_product_badge_action'); ?>  
                </div>        
                
                <!-- product countdown -->
                <div class="tp-product-countdown is-center-bottom">
                    <?php do_action('diego_product_countdown_action'); ?>
                </div>
                ...
                <div class="tp-product-action-2 tp-product-action-blackStyle tp-woo-action">
                    ...
                    <div class="tp-product-action-item-2 d-flex flex-column">
                        ...
                        <div class="tp-product-action-btn-2 tp-woo-add-to-compare-btn tp-woo-action-btn">
                            ...
                            <span class="tp-product-tooltip tp-product-tooltip-right tp-woo-tooltip"> <?php echo esc_html__('Add To Compare', 'diego'); ?></span>                                      
                        </div>
                        <?php endif; ?>
...
function diego_product_grid_type_2(){
    global $product;
    global $post;
    global $woocommerce;
    $rating = wc_get_rating_html($product->get_average_rating());
            ...
            <div class="tp-product-action tp-woo-action tp-woo-action-2">
                <div class="tp-product-action-item d-flex flex-column">
                    ...
                    <div class="tp-product-action-btn tp-woo-add-to-compare-btn tp-woo-action-btn">
                        <?php echo do_shortcode('[woosc]');?> 
                        <span class="tp-product-tooltip tp-woo-tooltip"> <?php echo esc_html__('Add To Compare', 'diego'); ?></span>                                      
                    </div>
                    <?php endif; ?>
                </div>
            </div>
...
function diego_product_grid_type_3(){
    global $product;
    global $post;
    global $woocommerce;
    $rating = wc_get_rating_html($product->get_average_rating());
...
function diego_product_grid_type_4(){
    global $product;
    global $post;
    global $woocommerce;
    $rating = wc_get_rating_html($product->get_average_rating());
...
function diego_product_grid_type_5(){
    global $product;
    global $post;
    global $woocommerce;
    $rating = wc_get_rating_html($product->get_average_rating());
...
function diego_product_grid_type_6(){
    global $product;
    global $post;
    global $woocommerce;
    $rating = wc_get_rating_html($product->get_average_rating());
...
function tp_smart_functions(){
    // smart quick view
    add_filter( 'woosq_button_html', 'diego_woosq_button_html', 10, 2 );
    function diego_woosq_button_html( $output , $prodid ) {
        $btntext = esc_html__("Quick View",'diego');
    ...
    // smart wishlist
    add_filter('woosw_button_html', 'diego_woosw_button_html', 10, 3);
    
    function diego_woosw_button_html($output, $id, $attrs){
    ...
    add_filter('woosw_button_text', 'diego_woosw_button_text', 10, 1);
    function diego_woosw_button_text($output){
        $btntext = esc_html__("Add To Wishlist",'diego');
        $action_layout = get_theme_mod('shop_grid_layout');
        if( $action_layout === "2" ||  $action_layout === "3" || $action_layout === "4" || $action_layout === "5"){
            $tooltip_position = '';
        }else{
    ...
    add_filter('woosw_button_text_added', 'diego_woosw_button_text_added', 10, 1);
    function diego_woosw_button_text_added($output){
    
        $btntext = esc_html__("View Wishlist",'diego');
        $action_layout = get_theme_mod('shop_grid_layout');
        if( $action_layout === "2" ||  $action_layout === "3" || $action_layout === "4" || $action_layout === "5"){
            $tooltip_position = '';
        }else{
    ...
    add_filter('woosc_button_htmls', 'diego_woosc_button_html', 10, 2);
    function diego_woosc_button_html($output, $id){
        global $product;
        $product = wc_get_product($id);
        $product_name = $product ? $product->get_name(): '';
    ...
    add_filter('woosc_button_texts', 'diego_woosc_button_text', 10, 1);
    function diego_woosc_button_text($output){
        $btntext = esc_html__("Add To Compare",'diego');
        $action_layout = get_theme_mod('shop_grid_layout');
        if( $action_layout === "2" ||  $action_layout === "3" || $action_layout === "4" || $action_layout === "5"){
            $tooltip_position = '';
        }else{
    ...
    add_filter('woosc_button_text_addeds', 'diego_woosc_button_text_added', 10, 1);
    
    function diego_woosc_button_text_added($output){
        $btntext = esc_html__("View Compare",'diego');
        $action_layout = get_theme_mod('shop_grid_layout');
        if( $action_layout === "2" ||  $action_layout === "3" || $action_layout === "4" || $action_layout === "5"){
            $tooltip_position = '';
        }else{
...
// product-content
if( !function_exists('diego_content_product_list') ) {
    function diego_content_product_list( ) {
        global $product;
        global $post;
        global $woocommerce;
        $rating = wc_get_rating_html($product->get_average_rating());
...
add_action( 'woocommerce_before_shop_loop_item_list', 'diego_content_product_list', 10 );
    ...
    global $product;
        ...
        if ( $product ) {
         ...
         if( $product->is_type( 'simple' ) ){
            $btntext = esc_html__("Add to Cart",'diego');
         } elseif( $product->is_type( 'variable' ) ){
            $btntext = esc_html__("Select Options",'diego');
         } elseif( $product->is_type( 'external' ) ){
            $btntext = esc_html__("Buy Now",'diego');
         } elseif( $product->is_type( 'grouped' ) ){
            $btntext = esc_html__("View Products",'diego');
         }
         else{
            $btntext = esc_html__("Add to Cart",'diego');
         } 
...
{
    global $product;
    $enable_buy_now = get_theme_mod('diego_product_single_buy_now_switch', true);
    if($enable_buy_now ){
        printf( '<button id="tp_wc-adding-button" type="submit" name="tp-wc-buy-now" value="%d" class="tp-product-details-buy-now-btn w-100 single_add_to_cart_button buy_now_button button alt">%s</button>', $product->get_ID(), esc_html__( 'Buy Now', 'diego' ) );
    }
}
